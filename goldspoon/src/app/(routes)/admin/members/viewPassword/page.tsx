...
export default function ViewPasswords() {
  ...
  const fetchAllMembers = async (pageNumber = 0) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/member/all?pageNumber=${pageNumber}&pageSize=${PAGE_SIZE}`, {
        headers: {
          "adminMemberId": 1,
        },
      });
  ...
  const fetchMemberById = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/member/${filterId}`, {
        headers: {
          "adminMemberId": 1,
        },
      });
