...
export default function InstallmentsPaid() {
  ...
  useEffect(() => {
    if (viewOption === "all") {
      const url = `https://goldspoon.in/api/installments?pageNumber=${currentPage - 1}&pageSize=${PAGE_SIZE}`;
      fetchData(url);
    }
  ...
  const getInstallmentsByMemberId = () => {
    ...
    setCurrentPage(1);
    const url = `https://goldspoon.in/api/installments?pageNumber=0&pageSize=${PAGE_SIZE}&memberNumber=${filterId}`;
    fetchData(url);
  };
