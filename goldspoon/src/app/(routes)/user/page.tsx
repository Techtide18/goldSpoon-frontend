...
export default function Home() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        const cardResponse = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/dashboard/member`, {
          params: {
      ...
      try {
        const withdrawalResponse = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/payout/withdrawalRequest/Details`, {
          params: {
            pageSize: 5,
            pageNumber: 0,
            memberNumber: session.user.name,
